...
export function FooterActions({ step, totalSteps, onPrev, onNext }: FooterActionsProps) {
  return (
    <div className="border-t border-slate-200 dark:border-slate-700 px-8 md:px-10 py-6 bg-slate-50 dark:bg-slate-900 flex gap-3 justify-between">
      {step > 1 && (
        <button
          onClick={onPrev}
          className="flex items-center gap-2 text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white font-medium transition-all duration-200 hover:gap-3"
        >
